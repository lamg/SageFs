  ...
  testSequenced <| testList "Package/Namespace Explorer" [
    ...
    testList "exploreNamespace" [
      ...
      <| fun _ ->
        ...
        let result = exploreNamespace ctx "System.IO" |> fun t -> t.Result
      ...
      <| fun _ ->
        let ctx = TestInfrastructure.sharedCtx ()
        let result = exploreNamespace ctx "System.Collections.Generic" |> fun t -> t.Result
      ...
      <| fun _ ->
        let ctx = TestInfrastructure.sharedCtx ()
        let result = exploreNamespace ctx "NonExistent.Namespace.Here" |> fun t -> t.Result
    ...
    testList "exploreType" [
      ...
      <| fun _ ->
        let ctx = TestInfrastructure.sharedCtx ()
        let result = exploreType ctx "System.IO.File" |> fun t -> t.Result
      ...
      <| fun _ ->
        let ctx = TestInfrastructure.sharedCtx ()
        let result = exploreType ctx "System.String" |> fun t -> t.Result
      ...
      <| fun _ ->
        let ctx = TestInfrastructure.sharedCtx ()
        let result = exploreType ctx "NonExistent.Type.Here" |> fun t -> t.Result
        Expect.stringContains result "No items found" "Should indicate nothing found"
    ]
  ]
